...
import { faAngleDown, faAngleUp, faCheckDouble, faListCheck, faSpinner } from '@fortawesome/free-solid-svg-icons';
  ...
  constructor(private mainTasklyService: MainTasklyService,
    ...
    this.mainTasklyService.decodeToken(this.tokenService.getToken()).pipe(
      concatMap((tokenData: UserProperties) => {
        ...
        return forkJoin({
          finished: this.mainTasklyService.getCompletedTask(this.userProperties.id),
          current: this.mainTasklyService.getCurrentTask(this.userProperties.id),
          all: this.mainTasklyService.getAllTaskCount(this.userProperties.id),
          completed: this.mainTasklyService.getCompletedTaskCount(this.userProperties.id),
          active: this.mainTasklyService.getActiveTaskCount(this.userProperties.id)
        }); 
      })
